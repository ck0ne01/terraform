...
resource "oci_core_security_list" "security_list" {
  ...
  dynamic "ingress_security_rules" {
    ...
    content {
      protocol = ingress_security_rules.key.protocol
      source   = ingress_security_rules.key.source
      tcp_options {
        max = ingress_security_rules.key.tcp_options.max
        min = ingress_security_rules.key.tcp_options.min
      }
    }
  }
}
